...
const accuracyKey = 'polished-cosmetology-accuracy';
...
function App() {
	...
	const [accuracy, setAccuracy] = useState(readAccuracy);
	...
	const topicQuestions = selectedTopic
		...
		: questions;
	const visibleQuestions = sessionLimit ? topicQuestions.slice(0, sessionLimit) : topicQuestions;
	const currentQuestion = visibleQuestions[questionIndex];
	const answered = selectedAnswer !== null;

	function startStudy(topicId = null, questionLimit = null) {
		const savedSession = readSavedSession();
		const shouldResume = savedSession?.topicId === topicId
			&& savedSession?.questionLimit === questionLimit
			&& window.confirm('Resume your saved session?');

		setSelectedTopic(topicId);
		setSessionLimit(questionLimit);
	...
	function chooseAnswer(answerIndex) {
		if (!answered) {
			...
			const nextAccuracy = {
				answered: accuracy.answered + 1,
				correct: accuracy.correct + (answerIndex === currentQuestion.answer ? 1 : 0),
			};
			setAccuracy(nextAccuracy);
			localStorage.setItem(accuracyKey, JSON.stringify(nextAccuracy));
		}
	}
	...
	function saveAndLeave() {
		localStorage.setItem(savedSessionKey, JSON.stringify({ topicId: selectedTopic, questionIndex, selectedAnswer, questionLimit: sessionLimit }));
		finishLeaveRequest();
	}
	...
	function readAccuracy() {
		try {
			return JSON.parse(localStorage.getItem(accuracyKey)) || { answered: 0, correct: 0 };
		} catch {
					...
					<section className="page-wrap study-page">
						<div className="study-header">
							<div>
								...
								<h1>{sessionLimit ? 'Quick practice' : selectedTopic ? getTopicLabel(selectedTopic) : 'Mixed review'}</h1>
							</div>
						...
						{sessionComplete && <CompletionDialog onContinue={continueStudying} onStop={goHome} />}
				</main>
			</div>
		);
	}
			...
			<main className="content">
				...
				<section className="page-wrap">
					<div className="welcome-row">
						...
						<div className="progress-ring"><strong>{getAccuracyPercent(accuracy)}%</strong><span>ACCURACY</span></div>
					</div>
					...
					<div className="stats-row">
						...
						<div><span>ACCURACY</span><strong>{getAccuracyPercent(accuracy)}%</strong><small>{accuracy.answered ? `${accuracy.answered} answered` : 'start practicing'}</small></div>
					</div>
					...
					<div className="tip-strip">
						...
						<button onClick={startQuickPractice}>Start now →</button>
					</div>
				</section>
			</main>
		</div>
...
function CompletionDialog({ onContinue, onStop }) {
	return (
		<div className="dialog-backdrop" role="presentation">
			<div className="save-dialog" role="dialog" aria-modal="true" aria-labelledby="complete-title">
				<h2 id="complete-title">Nice work today.</h2>
				<p>You finished this practice session. Do you want to keep going or stop for today?</p>
				<div className="dialog-actions"><button className="secondary-button" onClick={onStop}>Stop for today</button><button className="primary-button" onClick={onContinue}>Keep going</button></div>
			</div>
		</div>
	);
}
...
function getAccuracyPercent({ answered, correct }) {
